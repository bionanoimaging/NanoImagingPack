...
#        if animg is None:
        ...
        if self.ndims == 2 and animg.ndim == 2:  # here we can use the "standard" SVD representation
            ...
            self.kernels = image(self.kernels)
        else:
#            loss = lambda mykernelVec: GaussianLoss(joinFromVec(mykernelVec, kernellength, maxorders), animg)
    ...
    def join(self):
        ...
        """
        return join(self.kernels, None, self.dimStart)
    ...
    def convolve(self, obj, coefficientMap = None):
        ...
        :return:
        """
        maxorders = self.kernels.shape[0]
        res = 0.0
...
def join(kernels, kernelshape, dimStart):
    """
    ...
    ndims = len(dimStart) - 1
    maxorders = kernels.shape[0]
    img = 0.0
    for order in range(maxorders):
        myprod = kernels[order,dimStart[0]:dimStart[1]]
        for d in range(1, ndims):
            myprod = myprod * castdim(kernels[order,dimStart[d]:dimStart[d+1]], ndims, -d-1)
        img = img + myprod
    return image(img)
