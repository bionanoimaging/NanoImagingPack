...
def generate_grating(grating_para,phase_nr, num_phases,dim_slm= [1280, 1024],method = 'binary', blaze = 0,  periods = 1, binary_threshold = 0):
    ...
    if np.ndim(grating_para) == 1:
        ...
        grating = np.sin(xx(dim_slm)*k[0]+yy(dim_slm)*k[1]+phase_nr*np.pi*periods/num_phases)+1E-4;     
        if blaze >= 0:
            add_blaze = blaze*np.mod((xx(dim_slm)*k[0]+yy(dim_slm)*k[1]+phase_nr*periods*np.pi/num_phases+1E-4)/(2*np.pi),1.0);
        else:
            add_blaze = np.abs(blaze)*(1-np.mod((xx(dim_slm)*k[0]+yy(dim_slm)*k[1]+phase_nr*np.pi*periods/num_phases+1E-4)/(2*np.pi),1.0));
    else:
